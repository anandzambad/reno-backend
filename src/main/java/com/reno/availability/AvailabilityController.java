...
public class AvailabilityController {
    private final ContractorAvailabilityRepository repository; private final RedisAvailabilityService redis;
    ...
    public record AvailabilityRequest(@NotNull AvailabilityStatus status,@DecimalMin("-90") @DecimalMax("90") Double latitude,@DecimalMin("-180") @DecimalMax("180") Double longitude,@Positive Double serviceRadiusKm){}
    ...
    public ApiResponse<AvailabilityStatus> updateAvailability(@PathVariable long contractorId,@Valid @RequestBody AvailabilityRequest request){
        ...
        repository.save(entity); if(request.status()==AvailabilityStatus.OFFLINE)redis.markOffline(contractorId); return ApiResponse.ok(entity.getStatus());
    }
    ...
    public ApiResponse<List<NearbyContractor>> nearby(@RequestParam long serviceId,@RequestParam @DecimalMin("-90") @DecimalMax("90") double latitude,@RequestParam @DecimalMin("-180") @DecimalMax("180") double longitude,@RequestParam(defaultValue="10") @Positive double radiusKm,@RequestParam(defaultValue="20") @Positive int limit){
        List<NearbyContractor> list=redis.nearby(latitude,longitude,Math.min(radiusKm,50),Math.min(limit,50)).stream().filter(x->redis.isOnline(Long.parseLong(x.getContent().getName()))).map(x->{var p=x.getContent().getPoint();return new NearbyContractor(Long.parseLong(x.getContent().getName()),p.getY(),p.getX(),x.getDistance().getValue(),true);}).toList();
