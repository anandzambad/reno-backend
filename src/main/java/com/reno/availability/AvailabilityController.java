...
public class AvailabilityController {
 ...
 public ApiResponse<AvailabilityStatus> updateAvailability(@PathVariable @Positive long contractorId,@Valid @RequestBody AvailabilityRequest request){
  ...
  if(request.latitude()!=null&&request.longitude()!=null){entity.setLatitude(request.latitude());entity.setLongitude(request.longitude());entity.setLastSeenAt(Instant.now());if(request.status()==AvailabilityStatus.AVAILABLE)redis.upsertLocation(contractorId,request.latitude(),request.longitude());} repository.save(entity); if(request.status()!=AvailabilityStatus.AVAILABLE)redis.markOffline(contractorId); return ApiResponse.ok(entity.getStatus());
 }
 ...
 public ApiResponse<List<NearbyContractor>> nearby(@RequestParam @Positive long serviceId,@RequestParam @DecimalMin("-90") @DecimalMax("90") double latitude,@RequestParam @DecimalMin("-180") @DecimalMax("180") double longitude,@RequestParam(defaultValue="10") @DecimalMin("0.5") @DecimalMax("50") double radiusKm,@RequestParam(defaultValue="20") @Positive @org.hibernate.validator.constraints.Range(min=1,max=50) int limit){
  ...
  Set<Long> availableIds=repository.findByContractorIdInAndStatus(ids,AvailabilityStatus.AVAILABLE).stream().map(ContractorAvailabilityEntity::getContractorId).collect(java.util.stream.Collectors.toSet());
  ...
  List<NearbyContractor> list=candidates.stream().filter(x->{long id=Long.parseLong(x.getContent().getName());return availableIds.contains(id)&&eligible.contains(id);}).map(x->{var p=x.getContent().getPoint();return new NearbyContractor(Long.parseLong(x.getContent().getName()),p.getY(),p.getX(),x.getDistance().getValue(),true);}).toList(); return ApiResponse.ok(list);
 }
}
